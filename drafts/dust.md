...
If wind blowing through trees whispers, wind blowing over larger obstacles tends to moan. Presumably, this is not due to the bending moment of, e.g. a house or telephone pole. So what causes it? The answer takes us back to the dust devils. When wind blows around an object at low Reynolds number, it forms a cavity in the wake at sufficiently high ($\text{Re} > 90$) Reynolds number. That wake gets filled with eddies in a process called the [von Kármán vortex street](https://en.wikipedia.org/wiki/K%C3%A1rm%C3%A1n_vortex_street). For a wind of speed $u \sim 25 \text{ km/h} \approx 7 \text{ m/s}$. For an object of size $L \sim 1 \text{ m}$, the Reynolds number associated with the flow is

$$
\text{Re}_L = \frac{uL}{\nu} = \frac{7}{1.48 \times 10^{-5}} \approx 5 \times 10^5 \gg 90,
$$
