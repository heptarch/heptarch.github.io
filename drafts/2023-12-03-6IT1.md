...
of *Photinus carolinus* in the Great Smoky Mountains of North Carolina
...
The mathematics behind the synchronization is also very beautiful.
A simple approach to model the fireflies as *coupled oscillators*,
as pioneered by Kuramoto in 1975. Say we have $N$ fireflies, $i=1, 2,
\ldots, N$, whose brightness is governed by a phase $\theta_i$
according to $\sin(\theta_i)$,<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
	   ...
	   <\span>
and which naturally oscillates with frequency $\omega_i$, so
