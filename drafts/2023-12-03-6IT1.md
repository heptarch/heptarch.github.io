...
**December 5, 2023.** *6IT1: guarding art
  galleries; clairvoyant demons; counting prime factors;
...
<input type="checkbox"
	   ...
	   These recurrences reset a system to its initial state, thereby spontaneously reducing the entropy and violating
	   the second law of thermodynamics. We suppose the demon got
	   bored of waiting around—a recurrence will take much longer
	   than the current age of the universe—and decided, like Zermelo, to turn its
	   efforts to set theory.
	   </span>
...
This extrapolation will fail on some set of “unforseeable” times,
$$
...
Observe that $U$ cannot have an infinite ascending sequence of times, or
...
element.
We can argue that $U$ has measure zero by contraposition.
If $U$ has nonzero measure, we can pick an element $t_1$ so that $U_1
= U \cap (t_1, \infty)$ has nonzero measure; similarly, we can pick an
element $t_2 \in U_1$, then $t_3 \in U_2 = U \cap (t_2, \infty)$, and
so on. This form an ascending chain of times, so we are done.
$\blacksquare$
</div>

The demon is clairvoyant on almost all intervals $(-\infty, t)$, but with no
guarantees about how long that clairvoyance can last. <label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	We're being a bit unfair perhaps; we can almost always predict the
	value $f(t)$, of an arbitrary real function, based on the
	preceding values $f(x)$ for $x < t$.
	   </span> It would be
...
can't say when, where, or for how long.
