...
Ultimately, we need to know about physiology; it is the eye that is blue.
...
grey; line-height:1.5">
<i>Proof.</i>
For wavelength $\lambda$, Rayleigh scattering falls off as
$\lambda^{-4}$, while the blackody spectral intensity is
\[
B(\lambda) = \frac{\lambda^{-5}}{e^{C/T\lamda} - 1}
\]
for some constant $C$.
</div>
