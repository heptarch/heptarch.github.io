...
define the *coherence* $r$ as the amplitude of the phasor
...
same for all $j$, and $r = 1$, but otherwise interfere
destructively. In the extreme case of totally incoherent oscillators,
$r = 0$. We draw some cartoon phasors below:
...
unison.
You can see this in the simulation, but if you're interested, I'll
also show how to solve the the system analytically for the large-$N$ limit, following
...
uncorrelated <i>wanderers</i> with $|\omega| > Kr$, and uniformly random phase; and <i>followers</i> satisfying
\[
\frac{\omega}{K r} = \sin(\theta - \psi),
\]
where $re^{i\psi} = \sum_{j=1}^{N}e^{i\theta_j}$ is the phasor we
introduced above. The followers only appear at a critical coupling
$K_c = 2/\pi g(0)$.
</div>
...
<details>
  <summary>
   <i>Calculation.</i>$\,$ (Summary: turn a sum into an integral and
   solve a Fokker-Planck equation.)
  </summary>
  <p>
Consider the graph formed by the $n$ walls of the gallery, with
 walls as edges and the points they meet as vertices. Triangulate this graph,
