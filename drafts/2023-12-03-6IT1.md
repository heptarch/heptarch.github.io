...
& = \frac{1}{(k-1)!}\int_{\log 2}^{\log (n/2)} \text{d}s\,
\frac{n\log^{k-1}s}{s(\log n - s)} \\
& = \frac{1}{(k-1)!}\frac{n}{\log n}\int_{\log 2}^{\log (n/2)} \text{d}s\,
\log^{k-1}s \left[\frac{1}{s} + \frac{1}{\log n - s}\right],
\end{align*} 
where we integrate $x$ up to $n/2$,<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
	   ...
	   hence $\pi_(n/x) = 0$.
	   </span> make the substitution $s = \log (n/x)$ on the second
	   line, and use partial fractions on the third line.
</div>
...
Erdős and Kac's proof uses somewhat formidable sieve methods, and although simpler
