...
Without any coupling, the fireflies do not flash in sync, with zero
coherence, as we expect.
Let's add some attraction!
...
But we observe that when $\theta_j \approx \theta_i$,
$\sin(\theta_j - \theta_i)$ approximates our linear coupling, and
moreover, is always bounded by $1$. So we can try
...
This is the Kuramoto model! Let's turn on the coupling now. For $N =
100$ and $kN = 13 \times 10^{-3}$, we see *partial synchronization*:
