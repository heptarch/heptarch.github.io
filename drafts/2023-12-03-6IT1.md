...
primes; simulating fireflies; galactic AGI.*
...
5. <a href="#sec-6"><i>Neural scaling and black holes</i></a>
...
<details>
  <summary>
<i>Proof.</i>$\,$ (In short: prisoners see eventually identical
...
<details>
  <summary>
<i>Proof.</i>$\,$ (In short: the demon uses the prisoner's trick
...
<details>
  <summary>
<i>Calculation.</i>$\,$ (In short: take a product of spectra, differentiate, and set to zero.)
  </summary>
  <p>
...
<details>
  <summary>
<i>Proof.</i>$\,$ (In short: integrate by parts and use the prime
number theorem.)
  </summary>
  <p>
...
<details>
  <summary>
<i>Proof.</i>$\,$ (In short: remove a prime from the set, use
...
<details>
  <summary>
<i>Proof.</i>$\,$ (In short: apply the CLT to Landau's Poisson distribution.)
  </summary>
  <p>
...
<details>
  <summary>
   <i>Calculation.</i>$\,$ (In short: couple to the mean field, turn
...
<figure>
    <div style="text-align:center; padding: 15px"><img src
    ="/img/posts/firefly3.svg" width="300"/>
	</div>
	</figure>
...
evolutoin spreads this out to a large patch (when
...
## 6. <a href="#tbc">Neural scaling and black holes</a><a id="sec-6" name="sec-6"></a>

Suppose an alien civilization figures out how to turn a black hole
into a computer, or multiple black holes into a galaxy-wide distributed
network. Suppose, further, that want to create an "artificial
general intelligence" (AGI) by training something like a large
language model on this black hole computer.
This AGI would have two sets of limitations. First, there are
[fundamental bounds](https://arxiv.org/pdf/quant-ph/9908043.pdf) on
the operation of black hole computers due to the laws of physics;
second, there are [power law constraints](https://arxiv.org/abs/2001.08361)
on the performance of the model that are likely to be
[independent of the underlying architecture](https://arxiv.org/abs/2210.16859).
Our goal will be to relate them!<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   I would like to thank Lampros Lamprou for suggesting this problem.
	   </span>
	   
The basic idea of neural scaling laws is as follows. We have $D$
tokens in our dataset, $N$ trainable parameters in our model, and perform $S$
update steps using backpropagation.
Then the empirical loss (cross entropy) $L$ is a power law in any of
these terms when not "bottlenecked" by the other two. More precisely,
when we allow the model to converge ($S \to \infty$), then

$$
L(N, D) = \left[\left(\frac{N_c}{N}\right)^{\alpha_N/\alpha_D} + \frac{D_c}{D}\right]^{\alpha_D}
$$

for some constants $N_c, D_c$ and scaling parameters $\alpha_N
\sim 0.076$, $\alpha_D \sim 0.095$.
On the other hand, in the infinite data limit $D \to \infty$ (which is
effectively reached when we train on an internet-sized corpus), we have

$$
L(N, S) = \left(\frac{N_c}{N}\right)^{\alpha_N} + \left(\frac{S_c}{S^*}\right)^{\alpha_S},
$$

where $\alpha_S \sim 0.76$, and $S^* = S/(1+B^*/B)$ is an optimal number of steps,
determined by breaking our data into batches of size $B$ in a way we
discuss later.
We do not have an equivalent power law for $D$ and $S$, since the
infinite overfitting limit $N\to \infty$ is pathological.

Let's now think about how the aliens might train these with a network of black
holes.
The aliens are very clever and have some way to encode the model and
data into information they throw into the black holes, and parameter
updates they read from the Hawking radiation.

### References

- ["Scaling laws for neural language models"](https://arxiv.org/pdf/2001.08361.pdf)
  (2020), Jared Kaplan, Sam McCandlish, Tom Henighan, Tom B. Brown,
  Benjamin Chess, Rewon Child, Scott Gray, Alec Radford, Jeffrey Wu,
  and Dario Amodei.
- ["Ultimate physics limits to computation"](Ultimate physical limits
to computation) (1999), Seth Lloyd.
-  ["A solvable model of neural scaling laws"](https://arxiv.org/abs/2210.16859)
  (2022), Alexander Maloney, Daniel A. Roberts, and James Sully.
