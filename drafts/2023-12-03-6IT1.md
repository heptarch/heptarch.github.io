...
Without any coupling, the fireflies are not in sync, with zero
coherence, which is just what we expect. 
If firefly $i$ likes to align its phase with firefly $j$, then
$\theta_i$ tends to slow down if it is ahead of $\theta_j$
($\theta_j - \theta_i < 0$), and speed
up if it lags behind ($\theta_j - \theta_i > 0$).
We can implement this by adding a term to $\dot{\theta}_i$
