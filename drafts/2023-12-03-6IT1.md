...
eventually, and use AC to backwards complete.)
  </summary>
  <p>
...
Suppose you're waiting for an outcome and have no reliable information
about the timeline. Can you learn anything from the
wait time itself?<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   Based on a true story involving immigration, plane tickets, and
	   optimistic christmas plans.</span>
Suppose the (unknown) time it will actually take is $T$, and you have been
waiting $t$. The fraction of total time waited $x= t/T$
can be viewed as a random variable on $I = [0, 1]$.
Given that we knowing nothing about $T$, it seems reasonable to take
$x$ is uniformly distributed on $I$. The expected wait time is

$$
\mathbb{E}[T] = t\mathbb{E}\left[\frac{1}{x}\right] = t\int_0^1
\frac{\text{d}x}{x} = \infty.
$$
