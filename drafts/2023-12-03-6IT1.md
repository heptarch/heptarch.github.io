...
**Art gallery theory.** For a polygonal gallery of $n$ walls, $\lceil
n/3 \rceil$ guards are sufficient.

---

*Proof.* Consider the graph formed by the $n$ walls of the gallery, with
 walls as edges and the points they meet as vertices. Triangulate this graph, <label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   As Aigner and Ziegler point out, it's not obvious that a
 non-convex polygon has such a triangulation. To prove this, first
 observe that some vertex has an acute interior angle, since otherwise
 the walls would not close to form a loop.
	   </span>
i.e. introduce $n - 2$ new rectilinear edges that split the interior
 of the gallery into triangles.
■
