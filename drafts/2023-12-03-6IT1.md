...
Luckily, I found a hack to get the right answer without hard work. First, we use the usual
trick of finding peak color rather than integrating; second, we assume
the eye has adapted to the solar spectrum, and the spectral sensitive
$I_{\text{eye}}(\nu)$ equals the solar spectrum $I_{\odot}(\nu)$.
Of course, we don't only see direct solar radiation $I_{\odot}(\nu)$, but also some of
the light that gets scattered, with spectral density
$I_{\text{sc}}(\nu)$. A more refined hypothesis is that the sensitivity curve of the
eye is a *weighted geometric average* of the direct and diffuse
component, $ I^{(a)}_{\text{eye}}(\nu) = I_{\odot}(\nu)^a I_{\text{sc}}(\nu)^{1-a}$
for some index $a \in [0, 1]$.
Let's see what color of sky this predicts!

<div style="background-color: #202229 ; padding: 20px; border: 0px solid
grey; line-height:1.5; border-radius: 15px">
<span style="font-variant: small-caps">Spectral Adaptation.</span>
If the eye has an adapted sensitivity curve with index $a$, the
perceived spectral intensity peaks at a wavelength
\[
\lambda^* = \frac{hc}{(5+a) k_\text{B}T_\odot} \approx \frac{495}{1+a/5} \text{ nm}.
\]
where $hc/k_\text{B} \approx 14.4 \text{ mm}$ and $T_\odot \approx
5800 \text{ K}$ is the surface temperature of the sun.
</div>

<div style="background-color: #16171c ; padding: 20px; border: 0px solid
grey; line-height:1.5">
<i>Proof.</i>
For frequency $\nu$ and temperature $T$, the blackbody spectral
intensity is proportional to
\[
B(\nu) \propto \frac{\nu^{3}}{e^{C\nu/T} - 1}, \quad C = \frac{h}{k_\text{B}} \approx 4.8\times 10^{-11} \text{ s K}.
\]
Rayleigh scattering increases with frequency as $\nu^{4}$. On the
assumption that eye's sensitivity curve $S(\nu) = B(\nu)$, the perceived
spectral intensity $P(\lambda)$ is
\[
P(\nu) \propto \nu^4 B(\nu)S(\nu) = \nu^{4}B(\nu)^2 = \frac{\nu^{10}}{(e^{C\nu/T} - 1)^2} \propto \frac{x^{10}}{(e^{x} - 1)^2}
\]
where $x = C\nu/T$.
...
(x -  5)e^{x} + 5 = 0.
\]
This is a transcendental equation, with approximate solution $x
\approx 5$. It's common to consider the wavelength $\lambda = c/\nu$
of sunlight. Using the surface temperature $T \approx 5800 \text{ K}$,
we have
\[
\lambda_{\text{sky}} = \frac{c}{\nu} = \frac{Cc}{Tx} \approx \frac{hc}{5k_\text{B}T} \approx 495 \text{ nm}.
\]
This is blue with a green tinge, very close to the wavelength ($475 \text{ nm}$) we actually see! $\blacksquare$
</div>
