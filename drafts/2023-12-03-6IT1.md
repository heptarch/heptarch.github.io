...
instead, I'll finish the heuristic proof Kac may have presented at
Princeton in 1939.
...
where we write
\[
\pi_k(n) \sim ne^{-x}\frac{x^{k-1}}{(k-1)!},
\]
with $x = \log\log x$. 
...
\end{align*}
using $(\ref{CLT})$. This completes the heuristic proof!<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
	   ...
	   Landau's formula for $\pi_k(n)$, and we have neglected these entirely.
	   </span> $\blacksquare$
</div>
