...
<details>
  ...
  \[
   ...
   \label{kuramoto-fp}
	...
	equation. It has two solutions: the wandering solution $\rho = 1/2\pi$, with uniformly random
    phase, so $f$ is a constant; the synchronous solution, with
	\[
	\rho(\theta, t | \omega) = \delta\left(\theta - \psi - \sin^{-1}[K/\omega r]\right).
	\]
</p>
 </details>
 </div>
