...
<input type="checkbox"
       ...
       elementary” and omits it, which is usually a synonym for
       “hard”. <a
	   ...
	   </span> but corresponds to a probabilistic interpretation of
       the PNT where, instead of fixing $n$ and making divisibility by
       $p$ random, we fix $n$ and $m \leq n$ and make the primality of
       $m$ random.
...
precisely $\omega(m)  = k$, which equals $\pi_k(n)$ by definition. Thus,
\begin{align*}
\frac{1}{n}K_n(\alpha, \beta) & = \sum_{x + \alpha \sqrt{x} < k < x + \beta
\sqrt{x}} \frac{\pi_k(n)}{n} \\
& \sim \sum_{x + \alpha \sqrt{x} < k < x + \beta \sqrt{x}}
e^{-x}\frac{x^{k-1}}{(k-1)!} \\
