...
curve is a weighted average of these two,
\[
V(\nu) = \alpha I_{\odot} + (1- \alpha)I_{\text{sc}}(\nu)
\]
...
\[
V(x) \propto \left[\alpha + (1-\alpha) x^4\right] \frac{x^3}{e^x - 1}.
\]
The color of the sky has spectral intensity $V(x) I_{\text{sc}}(x)$.
