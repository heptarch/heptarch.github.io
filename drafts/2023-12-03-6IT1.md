...
If $U$ has zero measure, we can cover it by a family of intervals of
width $\epsilon/2^n$, $n \geq 1$, for any $\epsilon > 0$.
If it can't be covered, 
$\blacksquare$
</div>
...
<input type="checkbox"
       ...
       class="margin-toggle"/>
	   <span class="sidenote">
	   You might ask what it's building. I think the simplest answer
	   is an <i>error correcting code</i>, robust against any finite
	   number of errors. So far, there seems to be no practical use,
	   since while the errors do not spoil the code, they do spoil its
	   intended application to communication or prediction.
	   </span>
