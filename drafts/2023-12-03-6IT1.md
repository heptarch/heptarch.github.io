...
**December 5, 2023.** *6IT1: how to supervise children; clairvoyant demons; counting prime factors;
...
air molecules, and short wavelengths bounce more.
Blue has a short wavelength, so voilà!
But actually, violet has a shorter wavelength, so why isn't the sky
violet?
Well, the sun emits more blue light, so surely that does it?
[Do the calculation](https://hapax.github.io/physics/everyday/sky/),
and you find the sky should be green!
Ultimately, we need to know about physiology; it is the eye and not
the sky that is blue.

<figure>
    <div style="text-align:center; padding: 5px"><img src
    ="/img/posts/sky1.svg" width="300"/>
	</div>
	</figure>

To actually determine the perceived colour, we need to go and
look up the
[luminous efficiency functions](https://en.wikipedia.org/wiki/Luminous_efficiency_function),
[photopic vision](https://en.wikipedia.org/wiki/Photopic_vision) and
so on.
This is complex. But I discovered a physicist's hack: we use the
reasonable assumption that the sensitivity curve of the
eye has evolved to match the sort of light the sun produces.
We arrive at our guess for the perceived intensity by mutiplying the scattering
cross-section by the *square* of the sun's blackbody spectrum, and
find the maximum.

<div style="background-color: #202229 ; padding: 20px; border: 0px solid
grey; line-height:1.5; border-radius: 15px">
<span style="font-variant: small-caps">The Color.</span>
Bla
</div>
...
grey; line-height:1.5">
<i>Proof.</i>
For wavelength $\lambda$ and temperature $T$, the blackbody spectral
intensity is proportional to
\[
B(\lambda) = \frac{\lambda^{-5}}{e^{C/T\lambda} - 1},
\]
where $C = hc/k_\text{B} = 0.0144 \text{ m} \cdot \text{K}$ is a constant.
Rayleigh scattering falls off as $\lambda^{-4}$, so the perceived
spectral intensity of sky $P(\lambda)$ goes as
\[
P(\lambda) = \lambda^{-4}B(\lambda)^2 = \frac{\lambda^{-14}}{(e^{C/T\lambda} - 1)^2}.
\]
Differentiating with respect to $\lambda$ gives
\[
P'(\lambda) = -\left[7(e^{C/T\lambda} - 1) -\frac{C e^{C/T\lambda}}{T\lambda}\right]
\frac{2\lambda^{-15}}{(e^{C/T\lambda} - 1)^3} = .
\]
Setting this to zero, we find the maximum perceived intensity
$\lambda^*$ obeys
\[
0 = (x^* -  7)e^{x^*} + 7, \quad x = \frac{C}{T\lambda^*}.
\]
This is a transcendental equation, with no analytic solution, but as I
discuss [here](https://hapax.github.io/physics/everyday/sky/) it's
easy to show the approximate solution is $x^* \approx 7$ and hence
\[
\lambda^* \approx \frac{hc}{7k_\text{B}T}.
\]
</div>
