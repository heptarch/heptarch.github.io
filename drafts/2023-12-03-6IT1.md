...
define the *coherence* $r$ as the amplitude of

$$
re^{i\psi} = \frac{1}{N}\sum_{j=1}^N e^{i\theta_j}.
$$

The sum of exponentials on the right is called a *phasor*.
...
for a normalized coupling constant $K = kN$.
Below, we provide a firefly simulator, with randomized natural frequency and starting phase.
You can control the normalized coupling $K$ by clicking and dragging
the mouse up and down, and number of fireflies $N$ with the left and
right arrow keys. These are dynamically varied in the same simulation.
...
coherence is near zero. Above some critical threshold, we begin to see *partial
...
Eventually, all fireflies become *phase locked* and blink in
unison.
You can see this in the simulation, but if you're interested, I'll
also show how to solve the the system analytically for the large-$N$
limit, following [Acebrón et al.](https://home.iscte-iul.pt/~jaats/myweb/papers/new_kuras.pdf)
...
introduced above. The partially synchronized followers only appear above a critical coupling
$K_c = 2/\pi g(0)$.
</div>
