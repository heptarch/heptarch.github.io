...
\tag{3} \label{PNT+}
\]
for $\lambda = \log\log n$. This matches a Poisson distribution with parameter $x$.
</div>
...
& = \frac{n}{(k-1)!}\int_{\log \log 2}^{\log\log n} \text{d}s\,
s^{k-1}\\
& \sim \frac{n}{k!} (\log \log n)^k,
\end{align*}
making the substitution $s = \log t$ on the second line, and discard
the $s = 2$ contribution. $\blacksquare$
</div>
