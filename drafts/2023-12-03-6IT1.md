...
it produces an estimate astoundingly close to the real color of the sky.
...
Basically, ozone molecules absorb photons, get excited, then split
into oxygen molecules ($\text{O}_2$) or atoms ($\text{O}$).
This *photodissociation* process is messy, and allows a whole
spread of wavelengths to get absorbed, which is why all the yellow and
red seems to get leached out.
However, there are two peaks, at $\lambda = 575 \text{ nm}$ and
$\lambda = 603 \text{ nm}$. I picture these below:
...
It is precisely the nice sunset colors we like getting eaten by the
ozone!
What's left over is the faint blue that wasn't yet scattered. That's
what causes the middle band.
