...
The idea is to assume the eye adapted to be receptive to
...
scattered light, which is weaker that sunlight by a ratio of $15:85$. So a more
refined approximation is a weighted average,
$$
V = \alpha I_{\odot} + \beta I_{\text{diff}}
$$
for $\alpha=0.85$ and $\beta = 0.15$.
...
sun's temperature. Using $\alpha = 0.85$ this corresponds to a
wavelength of $\lambda^* \approx 470 \text{ nm}.$
</div>
...
peaks for perceived spectrum of scattered light are then
\[
h\nu^* = (7 - 2 \alpha) k_\text{B}T_{\odot} \quad \Longrightarrow \quad
\lambda^* = \frac{c}{\nu^*} = \frac{hc}{(7 - 2 \alpha) k_\text{B}T_{\odot}}.
\]
If we now plug in constants $hc/k_\text{B} = 14.4 \text{
...
This is relatively close to the effective perceived color, which has
$\lambda = 475 \text{ nm}$. To get a sense of the difference, we can
map these frequencies to color space using
[CIE 1931](https://en.wikipedia.org/wiki/CIE_1931_color_space), which
does know about physiology!

<figure>
    <div style="text-align:center; padding: 5px"><img src
    ="/img/posts/sky3.svg" width="250"/>
	</div>
	</figure>
...
different if we use wavelength! I must also sadly record that the
adaptive hypothesis is wrong; the photopic luminous efficiency
