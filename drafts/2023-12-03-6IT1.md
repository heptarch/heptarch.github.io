...
<details>
  ...
  <p>
  To show this is a QECC, first pick two
  orthogonal states from the code space,
  \[
   |\overline{T}_{1, 2}\rangle = \int \mathrm{d}g \, |g T_{1,2}\rangle.
   \]
   Now pick a region $K$, and apply two arbitrary errors on
   $K$, yielding corrupted states $\mathcal{E}_{1, 2}|\overline{T}_{1,
   2}\rangle$.
   Since these errors only act on $K$, the partial trace with respect
   to $K$ is unaffected,
   \[
   \rho_{1, 2} = \mbox{Tr}_K |\overline{T}_{1,2}\rangle\langle \overline{T}_{1,2}| = \mbox{Tr}_K [\mathcal{E}_{1, 2}|\overline{T}_{1,2}\rangle\langle \overline{T}_{1,2}|\mathcal{E}_{1, 2}^\dagger].
	\]
  </p>
  </details>
  </div>
...
Ammann lines.
Moreoever, inflation and deflation play an important role, and these
bear more than a passing resemblance to *renormalization*, which is
also expected to play a role in quantum gravity. In fact, some people
go so far as to sloganize

$$
\text{GR} = \text{RG},
$$

i.e. general relativity is the renormalization group.
At any rate, I suspect that, as Li and Boyle suggest, Penrose tilings
and their ilk may bumble out of the caves of recreational mathematics to play an
important role as toy models of quantum gravity in *flat* space.
