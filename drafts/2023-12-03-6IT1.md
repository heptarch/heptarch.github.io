...
\tag{3} \label{PNT+}
\]
Thus, $\pi_k/n$ is asymptotically Poisson distributed with parameter $\lambda = \log\log n$.
</div>
...
\]
From the bounds on $\Pi_k(n)/k!$, we have
\[
...
o(\Pi_k(n))$. Combining with $(\ref{Pi_k})$, we have
\[
\pi_k(n) -
\tau_k(n) \leq \Pi_{k-1}(n) \quad \Longrightarrow \quad \pi_k(n) \sim \tau_k(n) \sim \frac{\vartheta_k(n)}{k!\log n}.
\]
...
at large $n$, the nearby values of $\Omega(m)$ are normally
