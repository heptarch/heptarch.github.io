...
h\nu^* = (7 - 2 \alpha) k_\text{B}T_{\odot}
\]
as claimed. Plugging in numbers gives $\nu^* \approx 121(7-2\alpha)
\text{THz}$, which is in the right ballpark! $\blacksquare$
</div>

It's a bit hard to interpret this, so we turn it into a 
\[
\lambda^* = \frac{c}{\nu^*} = \frac{hc}{(7 - 2 \alpha) k T_{\odot}}.
\]
