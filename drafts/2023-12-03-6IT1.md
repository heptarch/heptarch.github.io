...
the demon *can* look into the future.
...
This extrapolation will fail on the set
$$
U = \{t \in \mathbb{R} : \hat{f}_t \neq \hat{f}_{t'} \text{ for all }
t' > t\}
$$
of “unforseeable” times.
...
First, note that $U$ cannot have an infinite ascending sequence of times, or
equivalently descending sequence of functions,
$$t_1
< t_2 < \cdots \quad \Longleftrightarrow \quad
\hat{f}_{t_1} \succ \hat{f}_{t_2} \succ \cdots,
$$
since for instance $\hat{f}_{t_1} \sim_{t_2} f$, but $\hat{f}_{t_2}$
is defined as the minimal such function so we must have $
\hat{f}_{t_1} \succ \hat{f}_{t_2} $. This
...
<input type="checkbox"
	   ...
	   $t_1$ with nonzero measure to the right, and so on.
	   </span>
$\blacksquare$
</div>

The demon is clairvoyant, but not on every interval, but with no
guarantees about how long its claivoyance can last. It would not be
wise to follow its investment advice! I doubt Feynman would find
Zermelo's demon worrying. In all the variants discussed in Hardin and
Taylor's
[monograph](https://qcpages.qc.cuny.edu/~rmiller/abstracts/Hardin-Taylor.pdf),
the demon's powers are also qualified up to some error set, which may
be finite, zero measure, or otherwise consisting of isolated points.
