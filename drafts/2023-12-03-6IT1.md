...
5. <a href="#sec-5"><i>Kuramoto fireflies</i></a>
...
## 5. <a href="#tbc">Kuramoto fireflies</a> <a id="sec-5" name="sec-5"></a>
...
sinusoidally, then some sort of interaction of *coupling* between
oscillators can cause them to synchronize.<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
...
Kuramoto (1975) introduced a cute model for this coupling which
does, in some cases, make them sync up.
...
define the *coherence* $r$ in the phasor

$$
re^{i\psi} = \frac{1}{N}\sum_{j=1}^N e^{i\theta_j}.
$$

The phases add perfectly constructively if and only if $\theta_j$ is the
same for all $j$, but otherwise will cancel, resulting in a smaller $r$.
...
If firefly $i$ likes to align its phase with firefly $j$, then
$\theta_i$ will slow down if it is ahead of $\theta_j$
($\theta_j - \theta_i < 0$), and speed up if it lags behind
($\theta_j - \theta_i > 0$).
A simple way to implement this is to add a term to $\dot{\theta}_i$
...
approximates a linear coupling, and this does respect periodicity. So
let's try
...
or, extending to all fireflies,

$$
\dot{\theta}_i = \omega_i + k\sum_{j=1}^N\sin(\theta_j - \theta_i).
$$

This is the Kuramoto model! Below, we provide a little firefly
simulator, with randomized natural frequency and starting phase.
By clicking and dragging on the canvas, you can control both
normalized coupling $K = kN$ (left-right) and number of fireflies $N$ (up-down):
...
coherence is near zero. As the coupling increases, you see *partial
synchronization*, though this can itself oscillate in a chaotic
fashion like the
[Belousov–Zhabotinsky reaction](https://en.wikipedia.org/wiki/Belousov%E2%80%93Zhabotinsky_reaction).
At high enough coupling, fireflies become *phase locked* and blink in
unison.<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   In fact, this is a second-order phase transition!
	   </span>
We solve the the system analytically, for the large-$N$ limit, in the box below.

<div style="background-color: #202229 ; padding: 20px; border: 0px solid
grey; line-height:1.5; border-radius: 15px">
<span style="font-variant: small-caps">Kuramoto Oscillators in the
Large-$N$ Limit.</span> Suppose natural frequencies have a probability
distribution $g(\omega)$ which is also even, $g(\omega) =
g(-\omega)$. As $N \to \infty$, there are two classes of firefly:
"wandering flashers" with uniform random phase, and natural frequency
$|\omega| > Kr$; and synchronized flashers satisfying
\[
\frac{\omega}{K r} = \sin(\theta - \psi),
\]
where $\psi$ is the phasor argument.
</div>
...
<details>
  <summary>
   <i>Calculation.</i>$\,$ (Summary: triangulate, 3-color, and place
   guards at one color. Click to expand.)
  </summary>
  <p>
Consider the graph formed by the $n$ walls of the gallery, with
