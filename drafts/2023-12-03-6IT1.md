...
<input type="checkbox"
       ...
       class="margin-toggle"/>
	   <span class="sidenote">
A linear order ($\prec, A)$ such that every nonempty set of $A$ has a least
element. We essentially just use the AC to pick the least element,
...
This extrapolation will fail on the set of “unforseeable” times
$$
...
First, note that $U$ cannot have an increasing sequence of times $t_1
< t_2 < \cdots$. This would imply
$
\hat{f}_{t_1} \succ \hat{f}_{t_2} \succ \cdots,
$
since for instance $\hat{f}_{t_1} \sim_{t_2} f$, but $\hat{f}_{t_2}$
is defined as the minimal such function, so it must be smaller. This
infinite descending chain is impossible, since $U$ has a least
element.
In turn, this implies that $U$ has measure zero.<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   If a set has nonzero measure, it has an infinite ascending
	   chain $t_1 < t_2 < \cdots$. We can simply pick an arbitrary
	   element $t_1$, then find some element to the right, and so on,
	   since deleting a point does not affect the measure.
	   </span>
$\blacksquare$
</div>
