...
<figure>
    <div style="text-align:center; padding: 5px"><img src
    ="/img/posts/prime3.svg" width="400"/>
	</div>
	</figure>
...
$$
\pi_k(n) \sim \frac{1}{(k-1)!}\frac{n}{\log n}(\log\log n)^{k-1} = ne^{-x}\frac{x^{k-1}}{(k-1)!}
\tag{3} \label{PNT+}
$$
where we set $x = \log\log x$. 
...
or more explicitly,
\[
\lim_{x\to\infty} \sum_{x + \alpha\sqrt{x} < k < x+ \beta\sqrt{x}} e^{-x}\frac{x^k}{k!} =\frac{1}{\sqrt{2\pi}}\int_\alpha^\beta \text{d}t\, e^{-t^2/2}.
\]
...
\end{align*}
using our earlier result. This completes the heuristic proof!<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
