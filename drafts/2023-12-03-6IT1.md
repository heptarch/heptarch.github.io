...
<div style="background-color: #16171c ; padding: 20px; border: 0px solid
grey; line-height:1.5">
<i>Proof.</i>To prove this, we use a set theory trick, and construct a
...
<input type="checkbox"
       ...
       class="margin-toggle"/>
	   <span class="sidenote">
A linear order $\prec$ such that every nonempty set has a least
element. We essentially just use the AC to pick the least element,
then its successor, and so on.
</span>
$\prec$ on the set of real functions using the AC. 
...
$$
of the equivalence class of $f$ as the minimal element.
The demon then simply guesses that $\hat{f}_t$ is the extension of $f$.
