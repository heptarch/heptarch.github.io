...
<figure>
    <div style="text-align:center; padding: 15px"><img src
    ="https://cdn.recreation.gov/public/2019/02/05/20/53/233374_a7d56e6d-b229-43f2-905e-95dfc905298f_1440.jpg" width="500"/>
	</div>
	</figure>

The mathematics is also very beautiful.
A simple approach pioneered by Kuramoto (1975) models the fireflies as
*coupled oscillators*. Say we have $N$ fireflies, $i=1, 2,
\ldots, N$, with brightness varying s $\sin(\theta_i)$<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   I don't think brightness is really sinusoidal, but rather a
	   sinusoid multiplied by a cutoff. I've added this to
	   the simulator for more realistic flashing.
<\span>
for a phase $\theta_i$.
If the firefly oscillates with natural frequency $\omega_i$, the phase
changes as

$$
\frac{\mathrm{d}\theta_i}{\mathrm{d}t} = \omega_i.
$$

In the presence of other fireflies, they do not oscillate at the
natural frequency, however, but will tend to sync up. Formally, we
introduce a simple attractive coupling which tends to align the phases
of firefly $i$ and $j$. For firefly $i$, it takes the form

$$
V_i = K \sin(\theta_j - \theta_i).
$$
...
function preload() {
  bgImage = loadImage(fireflies_data/bg2.png');
}
