...
so on.  This is hard work. Luckily, I discovered a physicist's hack
that gives us the right answer without doing all this hard work, using
the reasonable assumption that the eye evolved to respond to the light
the sun produces, i.e. its sensitivity curve matches the solar
spectrum.  Thus, perceived intensity is the *square* of the blackbody
spectrum, multiplied by a factor which captures scattering. We can use
this to approximate the color.
...
\[
B(\nu) \propto \frac{\nu^{3}}{e^{C\nu/T} - 1}, \quad C = \frac{hc^2}{k_\text{B}} \approx 4.3 \times 10^6 \text{ m}^2\text{ K/s}.
\]
Rayleigh scattering increases as $\nu^{4}$, so the perceived
...
\[
P'(x) \propto \left[5(e^{x} - 1) -x e^x\right]
\frac{2x^{11}}{(e^{x} - 1)^3} \quad \Longrightarrow \quad P'(x^*)
\propto (x^* -  5)e^{x^*} + 5 = 0.
\]
This is a transcendental equation, with no analytic solution, but as I
discuss [here](https://hapax.github.io/physics/everyday/sky/), you can
show the approximate solution is $x^* \approx 5(1 - e^{-5})$. Hence,
\[
