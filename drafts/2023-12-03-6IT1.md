...
parametrize it, and is different when we use wavelength. Second, the
sensitivity does not look like a long-tailed Maxwellian
distribution, but falls off much more quickly, so our adaptive
hypothesis is wrong.
Nevertheless, the peak frequency $\nu$ we've just calculated is very
close to the perceived color! Things are a little easier to interpret in terms of wavelength,
\[
