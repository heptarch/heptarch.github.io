...
It is remarkable that internal obstructions makes the task so
much harder; then again, perhaps we shouldn't be surprised that
supervising large groups of children gives rise to unsolvable
problems.
...
Richard Feynman refutes Banach-Tarski on the grounds that no *physical*
...
They argue that the AoC may have physical implications, using the following bizarre consequence:
...
If the prisoners are allowed to meet beforehand, know their position
on the number line, and can use the AoC,
...
grey; line-height:1.5">
<i>Proof.</i>
The strategy is simple. Consider the set $S = \{\text{R},
\text{B}\}^{\mathbb{N}}$ of possible sequences of red and blue
hats. Two sequences $s$ and $s'$ are *eventually equal* if they have
the form
$$
s = s_0 t, \quad s' = s'_0 t
$$
for some finite sequences $s_0, s_0'$ of equal length, and $t \in S$.
It's easy to see the relation of being eventually equal is an
equivalence relation, with equivalence classes $S_i$ for $i \in I$.
By the AoC, the prisoners can select representatives $s_i$ from each class.
$\blacksquare$
</div>
...
- ["The Axiom of Choice and the No-Signaling Principle"](https://arxiv.org/abs/2206.08467)
  (2023), Ämin Baumeler, Borivoje Dakić, Flavio Del Santo.
- 
