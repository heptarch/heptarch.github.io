...
where each $m_j > 0$ and $T$ is the total number of primes in the factorization.
We define
...
Let's plot a histogram of the distribution of $\omega(n)$ in a window
around $n_0$ for some increasingly large values of $n_0$, indicated
...
We can guess a few things from these plots.
First, we see that the average value of $\omega(n)$ grows
very very slowly; it is not even logarithmic (it doesn't scale with
the digits in $n_0$), so one might be tempted to guess it is *doubly logarithmic*, $\omega(n) \sim \log \log n$.
If we plot the average value of $\omega(n)$ against $\log \log n$, we find an almost perfectly straight line!

<figure>
    <div style="text-align:center; padding: 5px"><img src
    ="/img/posts/prime2.svg" width="250"/>
	</div>
	</figure>

The distribution around the mean also appears well-behaved; after
inspecting a few skewed distributions, it looks most like the
[Poisson distribution](https://en.wikipedia.org/wiki/Poisson_distribution).
For rate $\lambda$, the mean and variance are both $\lambda$, so we
can plot the variance and see if it agrees with the mean. At small
values they do not agree, but the the variance seems to approach the
mean, and it's plausible they agree at higher $n$.
Why would a Poisson distribution show up? The distribution

$$
P \sim \text{Poiss}(\lambda), \quad P(k) = e^{-k}\frac{\lambda^k}{k!}
$$

describe the probability an event occurs $k$ times, where each
occurrence is independent and the average number of times it appears
is $\lambda$. It's tempting to view a prime $p$ as being Poisson
distributed $\text{Poiss}(\lambda_p)$ over each number $n$, with $k$
the number of times it divides $n$ and $\lambda_p$ the expected number
of times it divides a random number.
If this is true, then we can express the average number of factors as

$$
\langle\omega(n)\rangle = \sum_{p< n} \lambda_p.
$$
