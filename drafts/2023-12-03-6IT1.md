...
**December 5, 2023.** *6IT1: how to supervise children; unreliably clairvoyant
  demons; why the sky is blue; randomness in prime factorization;*
...
machine learning (nominally six) on a semi-regular basis. These
may be new papers, old papers, discoveries, or shower thoughts; I'm hoping to
be brief and nerdily enthusiastic rather than thorough and technically deep.<label for="sn-1"
...
<input type="checkbox"
       ...
       class="margin-toggle"/>
	   <span class="sidenote">
That said, depth will vary. I'll try to give a subjective indication
of difficulty using stars: no stars mean accessible to a lay
audience; one star (＊) means advanced high school or undergraduate;
two stars (⁑) is upper undergraduate; three (⁂) is anything higher..
</span> Let's see how it goes!
...
4. <a href="#sec-4"><i>A prime Poisson process</i></a> ⁑
...
<div style="background-color: #16171c ; padding: 10px 20px; border: 0px solid
...
## 4. <a href="#tbc">A prime Poisson process</a> <a id="sec-4" name="sec-4"></a>
...
We'll focus on the second function, and try to answer: what is the behaviour of
...
distributed $\sim \text{Poiss}(\lambda_p)$ over each number $n$, for some
...
Assuming a few (very powerful) results, we
can do this sum.
...
noting that $C(x) = \pi(x)$, Abel summation $(\ref{abel})$ and $(\ref{PNT})$ give
\begin{align*}
	\sum_{p \leq n} \frac{1}{p} & = \frac{\pi(n)}{n} + \int_2^n \text{d}x \,
	\frac{\pi(x)}{x^2} \sim \frac{1}{\log(n)} + \int_2^n \,\frac{\text{d}x}{x\log x}
    \sim \log \log n,
	 \end{align*}
since $1/\log n \to 0$. The errors take more work, but
thankfully we won't need them!
$\blacksquare$
</div>
...
(CLT),
this will tend to a normal distribution $\mathcal{N}(\mu, \sigma^2)$ of mean and variance $nx$. For our
prime divisor-counting function $\Omega(n)$, this suggests that, as $n \to \infty$,

$$
\Omega(m) \sim \mathcal{N}(\mu, \sigma^2), \quad \mu = \sigma^2 \sim
\log\log n,
$$

when $m$ is taken in a vicinity of $n$. The normal number of factors
...
that $\Omega(m) =
k$, and $\tau_k(n)$ similiarly but with all prime factors required to
be different.
Then
\[
\pi_k(n) \sim \tau_k(n) \sim \frac{n}{\log n}\frac{(\log\log n)^{k-1}}{(k-1)!}.
\tag{3} \label{PNT+}
\]
...
grey; line-height:1.5">
<i>Proof.</i> We paraphrase the proof from <a
href="https://archive.org/details/introductiontoth00hard/page/n5/mode/2up">Hardy
and Wright</a> (Theorem 437).
Consider all sequences of $k$ primes $\leq n$, $(p_1, \ldots, p_k)$,
and let $c_k(m)$ be the number of sequences which produce $m$. Then we define
\[
\Pi_k(n) = \sum_{2 \leq m \leq n} c_k(m), \quad \vartheta_k(n) = \sum_{2 \leq m \leq n} c_k(m)\log m,
\]
which respectively count the total number of sequences with product
less than $n$, and the log-weighted sum.
By Abel summation $(\ref{abel})$, with $f(t) = \log t$ and $c_m =
c_k(m)$, we obtain
\[
\vartheta_k(n) = \Pi_k(n)\log n - \int_2^n \frac{\text{d}t}{t} \Pi_k(t). \tag{4} \label{vartheta}
\]
Since each valid sequence can be permuted at most $k!$ ways, and the
number of distinct sequences is smaller than sequences with
repetition, we have
\[
\tau_k(n) \leq \frac{1}{k!}\Pi_k(n) \leq \pi_k(n) \leq n, 
\]
so $\Pi_k(n) = O(n)$.
Performing the integral and inserting into $(\ref{vartheta})$,
\[
\int_2^n \frac{\text{d}t}{t} \Pi_k(t) = O(n) \quad \Longrightarrow \quad \Pi_k(n) = \frac{\vartheta_k(n)}{\log n} + O\left(\frac{n}{\log n}\right).
\]
From the bounds on $\Pi_k(n)$, we have
\[
\tau_k(n) \leq \frac{\vartheta_k(n)}{k!\log n} + O\left(\frac{n}{\log n}\right) \leq \pi_k(n). \tag{5} \label{Pi_k}
\]
The difference between $\pi_k(n)$ and $\tau_k(n)$ is entirely due to
sequences with repeated primes, and there are at most $\Pi_{k-1}(n)$ such
differences, since removing a repeated prime from the sequence leaves
its product less than $n$. We also show shortly that $\Pi_{k-1}(n) =
o(\Pi_k(n))$. Thus, $(\ref{Pi_k})$ implies
\[
\pi_k(n) = \tau_k(n) + o(\Pi_{k}(n)) \sim \frac{\vartheta_k(n)}{k!\log n}.
\]
Our goal now is to estimate $\vartheta_k(n)$.
In fact, we'll be sneaky and guess the answer we need, then prove it
by induction. Working backwards, we want
\[
\vartheta_k(n) \sim k n (\log\log n)^{k-1}.
\]
If true, this gives $\Pi_{k}(n)/\Pi_{k-1}(n) \sim \log\log n$ and hence $\Pi_{k-1}(n) = o(\Pi_k(n))$.
$\blacksquare$
</div>

Now that we have an asymptotic Poisson distribution, we can show that
at large $n$, the values of $\Omega(m)$ for $m$ near $n$ are normally
...
In some versions of the story, Erdős was dozing until then!
Erdős and Kac's derivation uses sieve methods, and although simpler proofs now exist we won't
...
<input type="checkbox"
       ...
       class="margin-toggle"/>
	   <span class="sidenote">
	   It's heuristic because we need to control the errors in
	   Landau's formula for $\pi_k(n)$. We've neglected them altogether!
	   </span> argument Kac may have presented at
Princeton in 1939, since it uses Landau's Poisson process to formalize the
loose argument we gave above.
It is one of the many gems in his
[beautiful book on probabilistic methods](https://store.doverpublications.com/0486821587.html).
...
within these bounds. As $n\to \infty$, this probability is normally
distributed, with mean and variance $\log\log n$, so that
\[
...
<div style="background-color: #16171c ; padding: 20px; border: 0px solid
grey; line-height:1.5">
<i>Proof.</i> We'll start with a precise statement of the CLT a Poisson variable
...
\]
Let's now apply it to Landau's Poisson primes $(\ref{PNT+})$, with
$x=\log\log n$.
We can split the count of $\Omega(m)$ into a count of the $m \leq n$ achieving
precisely $\Omega(m)  = k$, which is precisely $\pi_k(n)$. Thus, we
can use $(\ref{CLT})$ to deduce
\begin{align*}
...
\end{align*}
The number of prime factors really is normally distributed! $\blacksquare$
</div>
