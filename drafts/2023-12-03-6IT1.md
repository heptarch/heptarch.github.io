...
But a simpler, way to say this is that logical states remain
...
that, for $|u\rangle, |v\rangle \in \mathcal{C}$ and any errors
...
(We use the trace in case the errors make our states mixed.) With all
this setup, we can finally show that Penrose tilings naturally give
rise to a QECC, but one with some very curious properties.

<div style="background-color: #202229 ; padding: 20px; border: 0px solid
grey; line-height:1.5; border-radius: 15px">
<span style="font-variant: small-caps">Penrose Tilings as a
QECC.</span> Consider the Hilbert space $\mathcal{H}$ spanned by an
orthonormal basis $|T\rangle$ of states labelled by inequivalent
Penrose tilings. If $\mathcal{C} \subseteq \mathcal{H}$  is the space
spanned by states of the form
\[
|\tilde{T}\rangle = \int \mathrm{d}g \, |gT\rangle,
\]
where $\mathrm{d}g$ is the Haar measure over rigid motions of the
plane.
Then we have QECC that corrects errors in any finite region.
</div>
...
problems I find more interesting. First, is any erasure code
equivalent to a tiling? More precisely, suppose we have a code space
