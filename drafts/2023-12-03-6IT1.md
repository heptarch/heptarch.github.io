...
larger Hilbert space, such that some class of errors which corrupt the
...
<figure>
    <div style="text-align:center; padding: 15px"><img src
    ="/img/posts/qecc2.svg" width="250"/>
	</div>
	</figure>

More formally, we need an *encoding unitary* $\mathcal{U}: \mathcal{H}_0 \to
\mathcal{C}$ to turn logical states into physical states, an *error
channel* $\mathcal{E}: \mathcal{H} \to \mathcal{H}$ which corrupts
states, and a *recovery channel* $\mathcal{R}: \mathcal{H} \to
\mathcal{H}$ which undoes errors in the code subspace. Putting it
together, we have

$$
\mathcal{R} \circ \mathcal{E} \circ \mathcal{U} = \mathcal{U}.
$$
...
<figure>
    <div style="text-align:center; padding: 15px"><img src
    ="/img/posts/qecc1.svg" width="230"/>
	</div>
	</figure>

Another, perhaps simpler, way to say this is that logical states
remain distinguishable after certain errors. In quantum mechanics,
perfect distinguishability means the states are orthogonal, so we
could write

$$
\langle u | v\rangle = 0, \quad |u\rangle, |v\rangle \in \mathcal{H}_0
\quad \Longrightarrow \quad
\mbox{Tr}[(\mathcal{E}\mathcal{U})|u\rangle)^\dagger \mathcal{E}\mathcal{U})|v\rangle)] = 0.
$$
