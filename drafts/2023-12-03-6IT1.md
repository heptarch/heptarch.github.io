...
<details>
  ...
  discrete probability density $\rho$:
  \[
  r(t)e^{i\psi(t)} = \frac{1}{N}\sum_{j=1}^N e^{i\theta_j(t)} = \int_{-\pi}^\pi \mathrm{d}\theta\, e^{i\theta} \left[\frac{1}{N}\sum_{j=1}^N \delta(\theta-\theta_j(t))\right]
  = \int_{-\pi}^\pi \mathrm{d}\theta\, e^{i\theta} \rho(\theta, t | \omega_j),
  \]
  where we have rewritten the phase dependence in terms of the natural
  frequencies $\omega_j$.
  In the $N \to \infty$ limit, $\rho(\theta, t | \omega)$ becomes a
  continuous pdf, and the sum over index $j$ is replaced by an
  integral over $\omega$, weighted by the distribution $g(\omega)$:
  \[
  r(t)e^{i\psi(t)} = \int_{-\pi}^\pi \mathrm{d}\theta\, \int_{-\infty}^\infty \mathrm{d}\omega \, e^{i\theta} \rho(\theta, t | \omega) g(\omega).
  \]
  </p>
  As $N \to \infty$, 
 </details>
