...
<div style="background-color: #202229 ; padding: 20px; border: 0px solid
grey; line-height:1.5; border-radius: 15px">
<span style="font-variant: small-caps">Landau's Poisson Primes.</span>
Let $\pi_k(n)$ count
...
\tag{3} \label{PNT+}
\]
for $x = \log\log n$. This matches a Poisson distribution with parameter $x$.
</div>

<div style="background-color: #16171c ; padding: 20px; border: 0px solid
grey; line-height:1.5">
<i>Proof.</i> We will give a heuristic proof by induction; for a more careful demonstration, see <a
href="https://www.ma.ic.ac.uk/~bin06/M34PM16-Analytic-Number-Theory/m3pm16div.pdf">Nick
Bingham's notes</a>. First, the base case $k=1$ is the PNT
$(\ref{PNT})$.
Now assume it is true for $k$, and we wish to prove $k+1$. Our goal to
...
obtain
\begin{align*}
\pi_{k+1}(n) & \sim \frac{1}{(k-1)!}\int_1^n \text{d}u\,
\frac{n/u}{\log(n/u)}[\log\log (n/u)]^{k-1}\\
& = \frac{n}{(k-1)!}\int_0^{\log n} \text{d}t\,
\frac{\log^{k-1}} t}{t}\\
\end{align*}
making the substitution $t = \log (n/u)$ on the second line.
$\blacksquare$
</div>
