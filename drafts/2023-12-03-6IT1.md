...
First, note that $U$ cannot have an infinite ascending sequence of times,
$$t_1
< t_2 < \cdots \quad \Longleftrightarrow \quad
\hat{f}_{t_1} \succ \hat{f}_{t_2} \succ \cdots,
$$
or
descending sequence of functions, since for instance $\hat{f}_{t_1} \sim_{t_2} f$, but $\hat{f}_{t_2}$
is defined as the minimal such function, so we must have $ \hat{f}_{t_1} \succ \hat{f}_{t_2} $. This
...
The demon now seems to be clairvoyant! Or rather, it is clairvoyant
on most intervals, but with no guarantees about how long its
claivoyance can last. It would not be wise to purchase stocks. Once
again, I suspect Feynman would not find this incarnation of Zermelo's
demon physically relevant.
