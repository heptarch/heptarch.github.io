...
\[
P(\lambda) = \lambda^{-4}B(\lambda)^2 = \frac{\lambda^{-14}}{(e^{C/T\lambda} - 1)^2} \propto \frac{x^{14}}{(e^{x} - 1)^2}
\]
for $x = C/T\lambda$.
Differentiating with respect to $x$ and setting to zero gives
\[
P'(x) \propto \left[7(e^{x} - 1) -x e^x\right]
\frac{2x^{15}}{(e^{x} - 1)^3} \quad \Longrigharrow \quad P'(x^*)
\propto (x^* -  7)e^{x^*} + 7 = 0.
\]
