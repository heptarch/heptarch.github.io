...
\]
so $\Pi_k(n) = O(n)$.
It follows from $(\ref{vartheta})$ that
\[
...
it valid, i.e. with product less than $n$. Hence, $\pi_k(n) -
\tau_k(n) \leq \Pi_{k-1}(n)$. We show shortly that $\Pi_{k-1}(n) =
o(\Pi_k(n))$, and hence
\[
\pi_k(n) \sim \tau_k(n) \sim \frac{\vartheta_k(n)}{k!\log n}.
\]
