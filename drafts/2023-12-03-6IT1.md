...
does not lead to oscillatory behaviour, since the friction term
quickly dominates.
But we can observe that $\theta_j - \theta_i \approx \sin (\theta_j -
\theta_i)$ for $\theta_j \approx \theta_i$, and is bounded by $1$, so
will not wash out the natural frequency. This suggests we try

$$
\dot{\theta}_i = \omega_i + k \sin(\theta_j - \theta_i),
$$

or, extending to all fireflies and writing $k = K/N$ by convention,
