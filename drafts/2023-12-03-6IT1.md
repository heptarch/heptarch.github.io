...
inspecting a few skewed distributions, the best visual match is the
...
the numbers $1 \leq m\leq n$ with $k$ prime factors, i.e. $\omega(m) =
k$.
Then

$$
\frac{n}{\pi_k(n)} \sim \frac{1}{(k-1)!}\frac{1}{\log n}(\log\log
...
There are various proofs, but all seem to be unavoidably technical. So
instead, I'll finish with the heuristic<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
	   ...
	   </span> proof Kac may have presented at
Princeton in 1939.
