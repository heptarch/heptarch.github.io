...
mathjax: true
comments: true
title: "Six Impossible Things (1)"
short: "6IT1"
categories: [6IT]
date:  2024-01-05
---

**January 5, 2023.** *In the first issue of 6IT: supervising children
with obstacles; unreliably clairvoyant demons; why the sky is blue
(and why it isn't); a Poisson process for primes; sychronizing
fireflies; aperiodic tilings and quantum computers.*
...
<figure>
    <div style="text-align:center; padding: 15px"><img src
    ="/img/posts/penrose1.svg" width="480"/>
	</div>
	</figure>
...
<figure>
    <div style="text-align:center; padding: 15px"><img src
    ="/img/posts/penrose2.svg" width="480"/>
	</div>
	</figure>

Both results are straightforward to prove using two neat tools in the
analysis of aperiodic tilings: *Ammann lines* and *inflation/deflation*.
...
<figure>
    <div style="text-align:center; padding: 15px"><img src
    ="/img/posts/penrose3.svg" width="430"/>
	</div>
	</figure>
  ...
  we produce darts. Kites fill in the rest of the space automatically.

<figure>
    <div style="text-align:center; padding: 15px"><img src
    ="/img/posts/penrose3.5.svg" width="180"/>
	</div>
	</figure>
...
and rescale the $(1+\sqrt{5})/2$ to obtain a new, and unique, Penrose tiling.<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
...
Although we won't prove it, this process is reversible, so any tiling
  may be <i>deflated</i> by using the above dissection backwards.

<figure>
    <div style="text-align:center; padding: 15px"><img src
    ="/img/posts/penrose4.svg" width="400"/>
	</div>
	</figure>

This gives an easy proof of local indistinguishability. Given any finite
  patch of tiling $T$, we can deflate it until it is contained in a
  single vertex configuration, meaning a vertex and the rhombi that come
  together at that vertex. There are only a finite number of such
  vertex arrangements. It's easy to check that any vertex arrangement
  appears in any supertile, provided it is inflated enough. $\blacksquare$
  </p>
  </details>
  </div>
...
distinguishable after certain errors. In quantum mechanics, perfect
...
\mbox{Tr}[(\mathcal{E}_i|u\rangle)^\dagger \mathcal{E}_j |v\rangle]
= 0.
$$
...
<details>
  <summary>
   <i>Proof.</i>$\,$ (Take an overlap of states in the code space.)
  </summary>
  <p>
  To show this is a QECC for arbitrary finite erasures, first pick two
  orthogonal states from the code space,
  \[
   |\overline{T}_{1, 2}\rangle = \int \mathrm{d}g \, |g T_{1,2}\rangle.
   \]
   Now pick a region $K$, and apply two arbitrary error channels on
   $K$,
   \[
	\mathcal{E}_{1, 2}|\overline{T}_{1, 2}\rangle.
	\]
  </p>
  </details>
  </div>
...
This is very cool! Li and Boyle extend this to various other
scenarios, e.g. finite tilings and other dimensions. But there are two
questions I find interesting they don't really explore. First, can we reverse
this association, and obtain a tiling of some space from an erasure
code?
Suppose we have a Hilbert space $\mathcal{H}$ of "tilings", a code
space $\mathcal{C}$ of "symmetrized tilings", and a set of correctable
erasure channels $\mathcal{E}_i$, which perform arbitrary operations
on some subsystems $K_i$. This suggests that the "tiling" on $K_i$ is
recoverable from $K_i^c$.
...
$$
\begin{align*}
\mathcal{U}|00\rangle &= |0000\rangle + |1111\rangle \\
\mathcal{U}|01\rangle &= |0110\rangle + |1001\rangle \\
\mathcal{U}|10\rangle &= |1010\rangle + |0101\rangle \\
\mathcal{U}|11\rangle & = |1100\rangle + |0011\rangle.
\end{align*}
$$

This can correct an arbitrary error on a single qubit at a known location.
In this case, it's very clear what the associated "tiling" is: a
...
I suspect there is a nice way to generalize this!
...
For Penrose tilings, it is not only true that the tiling on a finite
region $K$ is determined by the complement $K^c$; in fact, any
*boundary neighbourhood* $\partial K$ of $K$ will do!
This is simply because we extend the Ammann lines and complete, as
with $K^c$.
It's cute that this theory is not only "holographic" in the sense that
what happens inside $K$ is determined by the tiling of $\partial K$,
but that we have a concrete mechanism for propagating that
information inward, namely the Ammann lines.
For more discusion of this line of thought, and various other fun
things, I recommend reading the paper!
