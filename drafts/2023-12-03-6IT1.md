...
of width $\Delta$ around $n$ for increasingly large values of
$n$. Only the mode is labelled:
...
<div style="background-color: #16171c ; padding: 20px; border: 0px solid
grey; line-height:1.5">
<i>Proof (sketch).</i> For a continuously differentiable function
...
The <a href="https://en.wikipedia.org/wiki/Prime_number_theorem">Prime Number Theorem</a>
...
\begin{align*}
    ...
    \sim \log \log n,
	 \end{align*}
since $1/\log n \to 0$. This completes the proof.<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   This is not watertight, since we need to bound the
	   corrections to our asymptotic formula for $\pi(x)$. But it's
	   simpler than any proof I've found online, and will suffice.
	   </span>
$\blacksquare$
</div>

Thus, we expect that if we average $\omega(n)$ over a large enough
...
we see good agreement, even at relatively small numbers! It looks as
...
If true, there is another remarkable consequence. As $x$
becomes large, $\text{Poiss}(x)$ can be viewed as a sum of $n$
i.i.d. variables $\text{Poiss}(x/n)$. By the
[Central Limit Theorem](https://en.wikipedia.org/wiki/Central_limit_theorem),
this will tend a normal distribution $\mathcal{N}(x, x)$ of mean and variance $x$. For our
...
[Erdős–Kac theorem](https://en.wikipedia.org/wiki/Erd%C5%91s%E2%80%93Kac_theorem).
The story goes that Mark Kac was giving a lecture at the Institute for
Advanced Study on a heuristic derivation of this result, which then
had the status of a conjecture. In Kac's words:

<div style="padding: 20px; border: 0px solid
grey; line-height:1.5; border-radius: 15px">
“I first stated the conjecture during a lecture in Princeton in March 1939.
Fortunately for me and possibly for mathematics, Erdos was in the audience and he
