...
**December 5, 2023.** *6IT1: guarding art
  galleries and after-school care; the demons of set theory; counting prime factors;
...
2. <a href="#sec-2"><span style="font-variant: small-caps">Zermelo's Demon and the Axiom of Choice</span></a>
3. <a href="#sec-3"><span style="font-variant: small-caps">Counting Factors with Erdős–Kac</span></a>
4. <a href="#sec-4"><span style="font-variant: small-caps">AI is the New Neusis</span></a>
5. <a href="#sec-5"><span style="font-variant: small-caps">Why the Sky is Blue</span></a>
...
This uses the AoC to split a perfect sphere into five
...
<input type="checkbox"
   ...
   additive, and we couldn't reassemble them to get something different!
	   </span> pieces, and reassemble them to form *two* spheres of
	   the same size!
...
<input type="checkbox"
       ...
       class="margin-toggle"/>
	   <span class="sidenote">
	   Which he mockingly calls "So-and-so's theorem of
...
They argue that the AoC may have physical implications, using a
...
here. The prisoners need to have an infinitely large memory to store
the representatives $s_i$, for instance.
...
Zermelo, originator of the AoC.<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   In fact, Zermelo already has an eponymous demon, associated with the
	   [Poincaré recurrence theorem](https://en.wikipedia.org/wiki/Poincar%C3%A9_recurrence_theorem).
	   Maybe this demon tired of thermodynamics and moved onto set
	   theory, like Zermelo himself.
	   </span>

<figure>
    <div style="text-align:center; padding: 15px"><img src
    ="/img/posts/aoc4.svg" width="380"/>
	</div>
	</figure>

This demon can use the AoC, and has a set of lucky coins; any time it
flips one, it learns the infinite history of coin outcomes before
it (analogous to prisoners looking backwards) and the number of flips
before the coin disintegrates (analogous to a prisoner learning their
number). Can the demon do better than blind chance in guessing the
remaining flips? The answer is no.

<div style="background-color: #202229 ; padding: 20px; border: 0px solid
grey; line-height:1.5; border-radius: 15px">
<span style="font-variant: small-caps">Zermelo's Demon.</span>
Whatever the history of the coin, or the number of flips left,
Zermelo's demon can never guess any behaviour of the coin with
better than random chance.
</div>

<div style="background-color: #16171c ; padding: 20px; border: 0px solid
grey; line-height:1.5">
<i>Proof.</i> Suppose the coin has $n$ flips left and a history of
flips $s \in \{\text{T}, \text{H}\}^{\mathbb{N}}$.
The demon can find the equivalence class $S_i$ of eventually equal
sequences, and choose the representative $s_i$ via the AoC. The question is
whether the outcomes $s_i(k)$, $0 \leq k \leq n - 1$, have any correlation
with the actual outcomes $s(k)$.
Since all possible outcomes $\{\text{T}, \text{H}\}^{n}$ for the
remaining flips are consistent with the choice of representative $s_i$,
$\blacksquare$
</div>
