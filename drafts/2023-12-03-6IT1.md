...
$$
P \sim \text{Poiss}(\lambda), \quad \mathbb{P}(k) = e^{-k}\frac{\lambda^k}{k!}.
$$
...
With some basic analytic number theory,<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
...
you may safely read the theorem and skip this (and the subsequent) proof.
	   </span> we
can do this sum.
...
<div style="padding: 20px 0px; border: 0px solid
grey; line-height:1.5; border-radius: 15px">
...
\[
\lim_{n\to\infty} \frac{K_n(\alpha, \beta)}{n} = \frac{1}{\sqrt{2\pi}}\int_{\alpha}^\beta \text{d}t \, e^{-t^2/2}.
\]
</div>
...
We can split the count of $\omega(m)$ into a count of $m$ achieving
precisely $\omega(m)  = k$, i.e. $\pi_k(n)$. Thus,
\begin{align*}
...
<input type="checkbox"
	   ...
	   Landau's result for $\pi_k(n)$, and we have neglected these entirely.
	   </span> $\blacksquare$
</div>
