...
sequences, and backwards complete with AC.)
  </summary>
  <p>
...
This is what we expect! In general, when $M = L$ can solve $(\ref{kuramoto-r})$ for $r$
numerically. For instance, when $K = 20 \times 10^{-3}$, $r
\approx 0.95$. The simulation doesn't quite match due to finite $N$
corrections, which you can read more about in
