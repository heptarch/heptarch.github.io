...
the time we can join a kite or a dart to a given edge. But Penrose
tilings have two remarkable properties I didn't previously know about:
1. (*Recoverability.*) The pattern in a finite region $K$ is uniquely determined by the
   pattern in $K^c$.
2. (*Indistinguishability*) Any finite patch of tiling $T$ is contained in *every* Penrose tiling of
the plane.
...
I think that (1) can be strengthened from the infinite complement
$K^c$ to a finite boundary patch $B$.
 Also, (2) can be strengthened to the statement that $T$ occurs
 *infinitely often*, with a spatial density independent of the ambient
 tiling. Let's prove these!
...
<details>
  <summary>
   <i>Proof.</i>$\,$ (In short: replace matchings with lines and .)
  </summary>
  <p>
  </p>
  </details>
...
As [Latham and Boyle (2023)](https://arxiv.org/pdf/2311.13040.pdf)
cutely observe, these properties sound a lot like an error-correcting code!
