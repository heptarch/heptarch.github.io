...
\[
\tau_k(n) \leq \frac{\vartheta_k(n)}{k!\log n} + O\left(\frac{n}{\log n}\right) \leq \pi_k(n).
\]
Since the difference in $\pi_k(n)$ and $\tau_k(n)$ is entirely due to
sequences with repeated primes, there are at most $\Pi_{k-1}(n)$ such
differences, since removing a repeated prime from the sequence leaves
it valid, i.e. with product less than $n$. We also show shortly that $\Pi_{k-1}(n) =
o(\Pi_k(n))$, and hence
\[
\pi_k(n) -
