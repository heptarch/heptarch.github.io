...
In fact, since a sum of independent Poisson-distributed variables is Poisson,<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   You can do the algebra, or prove it by considering what this
	   means: the total number of events is a sum over each type, with
	   mean given by the sum of means.
	   </span>
	   we expect
	   \[
	   \omega(n) \sim \text{Poiss}(\log \log n),
	   \]
	   so in particular the variance equals the mean.
	   We can check this empirically as well.
...
As $n$ gets larger, they become closer, with a
[vertical offset](https://en.wikipedia.org/wiki/Meissel%E2%80%93Mertens_constant)
of around $0.26$, which is the asymptotic difference between the sum of
reciprocal primes and the function $\log \log n$.
