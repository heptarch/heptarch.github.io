...
Penrose tilings. If $\mathcal{C} \subseteq \mathcal{H}$ is spanned by states of the form
\[
|\overline{T}\rangle = \int \mathrm{d}g \, |gT\rangle,
\]
where $\mathrm{d}g$ is the Haar measure over rigid motions of the
plane, then it forms the code space of a QECC that can correct arbitrary errors in any finite region.
</div>
...
this association, and obtain a tiling of some space from an erasure code?
