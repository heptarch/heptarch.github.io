...
<details>
  ...
  <p>
  Our phasor can be written as a convolution over a
  discrete probability density:
  \[
  re^{i\psi} = \frac{1}{N}\sum_{j=1}^N e^{i\theta_j} = \int_{-\pi}^\pi \mathrm{d}\theta\, e^{i\theta} \left[\frac{1}{N}\sum_{j=1}^N \delta(\theta-\theta_j)\right].
  \]
 </p>
 </details>
 </div>
