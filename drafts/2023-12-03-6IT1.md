...
so on.  This is hard work, and I am a lazy physicist. Luckily, I discovered a hack
that gives the right answer without hard work; we just make
the reasonable assumption that the eye evolved to respond to the
spectrum of light the sun produces.
Let's see what color of sky this predicts!

<div style="background-color: #202229 ; padding: 20px; border: 0px solid
grey; line-height:1.5; border-radius: 15px">
<span style="font-variant: small-caps">Spectral Adaptation.</span>
If the sensitivity curve of the eye perfectly matched the sun, the
perceived color of the sky would be $\lambda = 495 \text{ nm}$.
</div>
...
Of course, we don't only see direct solar radiation, but also some of
the light that gets scattered, with a split of about $85\%$ to
$15\%$. A more refined hypothesis is that the sensitivity curve of the
eye is a *weighted geometric average* of the direct and diffuse
component,
\[
S(\nu) \propto B(\nu)^{0.85} \big(\nu^4 (\nu)\big)^{0.15} = \nu^{0.6} B(\nu).
\]
The same calculation then leads to $\lambda_{\text{sky}} = 465 \text{
nm}$. Not too shabby!

<figure>
    <div style="text-align:center; padding: 5px"><img src
    ="/img/posts/sky2.svg" width="280"/>
	</div>
	</figure>
