...
An attractive phase coupling to firefly $j$ tends to slow
$\phi_i$ down if it is ahead of $\phi_i$, and otherwise speed it up. A
simple way to implement this is to add a term to $\dot{\theta}_i$
...
For a constant $k > 0$, this is attractuve, since $\theta_i$ speeds up
if it lags behind $\theta_j$, slows down if it's ahead, and does so
