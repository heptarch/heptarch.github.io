...
above each histogram. The mode, i.e. the most frequently occurring
value of $\omega(n)$, is shown below:
...
summation formula</a> states that, for a continuously differentiable function
$f(x)$ on $[\alpha, \beta]$, and a sequence $c_n$,
\[
...
primes. Setting $c_n = \delta_{\mathbb{P}}(n)$ and $f(n) = 1/n$, and
noting that $C(x) = \pi(x)$, Abel summation gives
\begin{align*}
	\sum_{p \leq n} \frac{1}{p} & = \frac{\pi(n)}{n} + \int_1^n \text{d}x \,
	\frac{\pi(x)}{x^2} \\
	 & \sim \frac{1}{\log(n)} + \int_1^n \,\frac{\text{d}x}{x\log x}
	 \\
	 & = \frac{1}{\log(n)} + \log\log n \sim \log \log n.
\end{align*}
$\blacksquare$
</div>

A sum of Poisson distributions is Poisson, so we arrive at the hypothesis that

$$
\omega(n) \sim \text{Poiss}(\log\log n).
$$

Plotting the mean and variance $\omega(n)$ against $\log \log n$, we see an
almost perfect correspondence! The Poisson process has variance equal
to its mean. Plotting the sample variance for our data, we see that at
small values it does not agree with the mean, but gets closer as $n$ increases.
...
It's plausible, then, that this equation holds for large $n$.
