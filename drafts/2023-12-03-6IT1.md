...
But we also see diffuse illumination of intensity $I_{\text{diff}}$ from
scattered light, which is weaker by a ratio of $15:85$, so a more
refined approximation is a weighted average,
$$
V = \alpha I_{\odot} + \beta I_{\text{diff}}
$$
for some $\alpha=0.85$ and $\beta = 0.15$.
Sunlight tends to be intense than diffuse light by a factor of
$85:15$, so we take $\alpha = 0.85$.
...
sun's temperature. Converting to a wavelength, using $\alpha = 0.85$, and plugging in constants gives
\[
\lambda^* \approx 470 \text{ nm},
\]
in good agreement with the perceived color of $\lambda = 475 \text{ nm}$.
</div>
...
solution</a> $x \approx 7 - 2 \alpha$. The corresponding frequency and wavelength
peaks for perceived spectrum of scattered light is then
\[
...
\[
\lambda^* = \frac{14.4 \text{ mm}}{(7-2 \cdot 0.85)5800} \approx 470 \text{ nm},
\]
very close to the effectively perceived color.
$\blacksquare$
</div>
...
totally different when we use spectral energy per unit
wavelength instead of frequency! It's also interesting to note that
our adaptive hypothesis is wrong; the photopic luminous efficiency
function falls off much more quickly than a blackbody distribution.
Still, the hack works, and as a theoretical physicist I invoke my
right to leave it at that.
