...
equivalence class of $f$ with respect to the relation $\sim_t$, and
simply try to extend $f$ using $\hat{f}_t$.
This will fail on the set of "unforseeable" times
$$
U = \{t \in \mathbb{R} : \hat{f}_t \neq \hat{f}_{t'} \text{ for all }
t' > t\}.
$$
These are times for which $\hat{f}_t$ fails to agree with $f$ at all
$t' > t$, or equivalently, $\hat{f}_{t'}$. Our goal now is to show the
set $U$ is measure zero.
$\blacksquare$
</div>
