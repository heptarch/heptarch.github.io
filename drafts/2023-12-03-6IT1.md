...
$I_{\odot}(\nu)$, where $\nu$ is frequency.
But we don't only see direct solar radiation, but also
the light that gets scattered, with spectral density
$I_{\text{sky}}(\nu)$. A second approximation is that the sensitivity
curve is a weighted average of these two, $V(\nu) = \alpha I_{\odot} +
\beta I_{\text{sky}}(\nu)$,
...
If the eye adapts to a mixture of direct and scattered sunlight with
weight $\alpha$, the peak frequency is
\[
\nu = (7 - 2 \alpha) \frac{k_\text{B}T_{\odot}}{h},
\]
where $k_\text{B}$ is Boltzmann's constnat, $T_\odot$ is the surface
temperature of the sun, and $h$ is Planck's constant.
</div>
...
\[
V(x) = \alpha I_{\odot} + (1- \alpha)I_{\text{sc}}(\nu) \propto \left[\alpha + (1-\alpha) x^4\right] \frac{x^3}{e^x - 1},
\]
since using the dimensionless $x$ variables ensures the constants of
proportionality are equal.
The sky has spectral intensity $I_{\text{sky}}(x)$, and thus perceived
spectral intensity $V(x)I_{\text{sky}}(x)$.
...
\approx 7 - 2 \alpha$. This gives rise to a peak frequency
\[
\nu^* = (7 - 2 \alpha) \frac{k_\text{B}T_{\odot}}{h} 
\]
