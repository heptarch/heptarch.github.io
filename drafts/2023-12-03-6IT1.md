...
<input type="checkbox"
       ...
       class="margin-toggle"/>
	   <span class="sidenote">
	   It's heuristic because we haven't kept track of the errors
...
\]
We can split the count of $\Omega(m)$ by the values it assumes, i.e. into a count of the $m \leq n$ achieving
precisely $\Omega(m)  = k$, which is given by $\pi_k(n)$.
...
\end{align*}
This proof isn't watertight, but it is a more rigorous way to present
our argument. $\blacksquare$
</div>

So, as $n$ gets large, the number of primes factors is distributed
according to a smooth Bell curve around $n$. Perhaps the primes play a
game of chance, as Kac puts it, but in the long run the house always wins.
