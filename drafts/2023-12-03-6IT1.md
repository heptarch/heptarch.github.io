...
$\omega(n)$? A good place to start is numerics. Let's plot a histogram of the distribution of $\omega(n)$ in a window
of width $\Delta = \min\{n, 10^3\}$ around $n = 10^2, 10^4, 10^6,
10^8, 10^{10}$:
...
The mode is the only marked point on the $x$ axis. Evidently, it grows
very slowly.<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   As one anonymous wag said of a related function, it “has been
...
\[
	\lim_{x \to \infty} \sum_{\alpha\sqrt{x} < k - x < \beta\sqrt{x}} \frac{x^k}{k!} = \frac{1}{\sqrt{2\pi}}\int_\alpha^\beta \text{d}t\, e^{-t^2/2}.
\]
</div>
