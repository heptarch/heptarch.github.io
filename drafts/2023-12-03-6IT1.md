...
\]
We can split the sum over values of $\omega(m)$ into a sum over $k$ in
$\pi_k(n)$, so that
\begin{align*}
K_n(\alpha, \beta) & = \sum_{x + \alpha \sqrt{x} < k < x + \beta
\sqrt{x}} \pi_k(n) \\
& \sim \sum_{x + \alpha \sqrt{x} < k < x + \beta \sqrt{x}} \frac{1}{(k-1)!}\frac{n}{\log n}(\log\log n)^{k+1}
\end{align*}
</div>
