...
even, then the phase-locked population only appear above a critical coupling
$K_c = 2/[\pi g(0)]$.
</div>
...
<details>
  ...
  \[
   ...
   \label{kuramoto-fp}
	...
	\begin{align}
	r = \mathbb{E}_\rho\left[e^{i(\theta-\psi)}\right] & =
	\int_{-\pi/2}^{\pi/2} \mathrm{d}\theta \int_{-\infty}^\infty
	\mathrm{d}\omega \, \delta\left(\theta - \psi -
	\sin^{-1}[\omega/K r]\right) g(\omega) + \int_{-\pi}^{\pi} \mathrm{d}\theta \int_{-\infty}^\infty
	\mathrm{d}\omega \, \frac{Cg(\omega)}{|\omega + Kr \sin(\psi-\theta)|}.
	\end{align}
</p>
 </details>
 </div>
