...
**December 5, 2023.** *6IT1: guarding art
  galleries and after-school care; clairvoyant demons; counting prime factors;
...
Though the AC is very intuitive—we can simply select elements from nonempty
...
This uses the AC to split a perfect sphere into five
...
The fact that Zermelo's demon cannot do anything spooky with future
coin flips makes the AC seem less physically disturbing. But there is
another variant due to [Hardin and Taylor](https://www.jstor.org/stable/27642415)
which shows the demon *can* look into the future.
...
behaviour on $(-\infty, t')$ for some $t' > t$, except for $t$
on a set $U$ of measure zero.
</div>

<div style="background-color: #16171c ; padding: 20px; border: 0px solid
grey; line-height:1.5">
<i>Proof.</i> Consider the equivalence relation on functions $f:
\mathbb{R}\to \mathbb{R}$ given by
$$
f \sim_t g \quad \Longleftrightrarrow \quad f(x) = g(x) \text{ for all } x
\leq t.
$$
Using the AC, the demon can choose a representative $\hat{f}_t$ of the
equivalence class of $f$ with respect to the relation $\sim_t$.
Define the set of "unforseeable" times by
$$
U = \{t \in \mathbb{R} : \hat{f}_t \neq \hat{f}_{t'} \text{ for all }
t' > t\}.
$$
These are times for which the demon cannot extend the behaviour of $f$
on $(-\infty, t)$ to $(-\infty, t')$ for any $t' > t$.
$\blacksquare$
</div>

When $f(t)$ is interpreted as the behaviour of some function in time,
it appears the demon can almost always see some distance into the
future, just using the AC!
