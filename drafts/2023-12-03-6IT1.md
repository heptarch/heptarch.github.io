...
audience; five points (＊) is undergraduate; six points (✶) is upper
undergraduate or graduate; eight points (✸) is graduate;
twelve points (✹) is research level; and a $16$-pointed star (✺) is an
open problem.
...
## 4. <a href="#tbc">Poisson primes</a> <a id="sec-4" name="sec-4"></a>
...
We'll focus on the latter, and ask: what is the behaviour of
...
Why would a Poisson distribution show up? $\text{Poiss}(\lambda)$
counts the number of times an event occurs, where each occurrence is
...
\[
	\sum_{\alpha < n \leq \beta} c_n f(n) = C(\beta)f(\beta) - \int_\alpha^\beta \text{d}x \, C(x)f'(x), \tag{1}\label{abel}
\]
...
\[
\pi(x) = \sum_{1 < n \leq x} \delta_{\mathbb{P}}(n) \sim \frac{x}{\log x} \tag{2}\label{PNT}
\]
...
If true, this has a remarkable consequence. As $n$ becomes large,
...
Let's turn now to what can be rigorously proven. The PNT $(\ref{PNT})$
was first derived rigorously in 1896 (by Hadamard and de la Vallée Poussin
independently) but in 1903, Edmund Landau gave a simpler proof and
made the following generalization. Let $\pi_k(n)$ count
the numbers $m$, $1 \leq m\leq n$ with $k$ distinct prime factors, i.e. such
that $\omega(m) =
k$.
Then

$$
\frac{\pi_k(n)}{n} \sim \frac{1}{(k-1)!}\frac{1}{\log n}(\log\log
n)^{k-1} = e^{-x}\frac{x^{k-1}}{(k-1)!}
\tag{3} \label{PNT+}
$$

for $x = \log\log n$.
This is a Poisson distribution with parameter $x$!
The proof isn't easy,<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
id="sn-1"
class="margin-toggle"/>
<span class="sidenote"> In his book, Kac calls it “entirely
elementary” and omits it, which is usually a synonym for “hard”. <a
href="https://www.ma.ic.ac.uk/~bin06/M34PM16-Analytic-Number-Theory/m3pm16div.pdf">Nick
Bingham's proof</a> for the case $k=2$ corroborates this suspicion.
</span> but there is a nice way to think about where this comes from.
To tote up $\pi_k(n)$, we need to find sets of $k$ primes whose
product is less than $n$.
Only primes less that $n$ will do, and there are $\pi(n)$ of them.
The total number of sets, without the product constraint, is

$$
\binom{\pi(n)}{k} = \frac{(n/log n)!}{k! (n/log n - k)!}
$$

Erdős–Kac Theorem.
Its genesis is legendary.
In Mark Kac's words:
...
The proof of Erdős and Kac is somewhat technical, and although simpler
proofs exist, we won't bother. Instead, I'll finish with the heuristic<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
	   ...
	   Landau's formula for $\pi_k(n)$, which we neglect entirely.
	   </span> argument Kac may have presented at
Princeton in 1939, since it simply formalizes the loose argument we have
already given.
...
grey; line-height:1.5">
<i>Proof.</i> Let's state the CLT for consider a Poisson variable
...
or more explicitly,
\[
\lim_{x\to\infty} \sum_{x + \alpha\sqrt{x} < k < x+ \beta\sqrt{x}} e^{-x}\frac{x^k}{k!} =\frac{1}{\sqrt{2\pi}}\int_\alpha^\beta \text{d}t\, e^{-t^2/2}. \label{CLT} \tag{4}
\]
Let's now apply it to Landau's Poisson primes $(\ref{PNT+})$.
...
\infty}\sum_{\log\log n + \alpha \sqrt{\log\log n} < k < \log\log n + \beta
\sqrt{\log\log n}} \frac{\pi_k(n)}{n} \\
& = \lim_{x\to\infty} \sum_{x + \alpha \sqrt{x} < k < x + \beta \sqrt{x}}
e^{-x}\frac{x^{k-1}}{(k-1)!} \\
& = \frac{1}{\sqrt{2\pi}}\int_\alpha^\beta \text{d}t\, e^{-t^2/2},
\end{align*}
using $(\ref{CLT})$. This more or less the argument we
used above, but made (more) rigorous by Landau's prime Poisson process! $\blacksquare$
</div>
