...
incoherent oscillators, with $|\omega| > Kr$ and uniformly random
phase; and coherent oscillators, satisfying
\[
...
<details>
  ...
  \[
   ...
   \label{kuramoto-fp}
	...
	Then $(\ref{kuramoto-fp})$ is a noise-free Fokker-Planck
	equation. It has two solutions: the incoherent solution $\rho = 1/2\pi$, with uniformly random
    phase, so $f$ is a constant; and the fully coherent solution, with
	\[
	\rho(\theta, t | \omega) = \delta\left(\theta - \psi - \sin^{-1}[\omega/K r]\right),
	\]
	where the individual oscillators are phase locked, $\dot{\theta}=0$.
</p>
 </details>
 </div>
