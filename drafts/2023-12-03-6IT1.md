...
as pioneered by Kuramoto in 1975. Say we have $N$ fireflies, and assign
firefly $i$ a phase $\theta_i$, with brightness $\sin(\theta_i)$.<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
       class="margin-toggle"/>
	   <span class="sidenote">
	   Really, brightness should be positive, so we could try
	   multiplying $(\sin(\theta)+1)/2$ by maximum overall
	   brightness. This isn't really how fireflies behavem though, so
	   I've introduced
	   <\span>
