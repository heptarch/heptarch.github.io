...
First, we can see that the average value of $\omega(n)$ grows
very very slowly; it is not even logarithmic, so one might be tempted
to guess it is *doubly logarithmic*, $\omega(n) \sim \log \log n$.
If we plot the average value of $\omega(n)$ against $\log \log n$, we
find an almost perfectly straight line of slope $1$!

<figure>
    <div style="text-align:center; padding: 5px"><img src
    ="/img/posts/prime1.svg" width="300"/>
	</div>
	</figure>
