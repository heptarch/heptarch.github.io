...
<details>
  <summary>
   ...
   sums to integrals, solve Liouville's equation.)
  </summary>
  <p>
  ...
  \[
	r e^{i\psi} = \mathbb{E}_{\rho}[e^{i\theta}], \quad \rho(\theta, t) =
	\int_{-\infty}^\infty \mathrm{d}\omega\,\rho(\theta, t | \omega)
	\sum_{j=1}^N \delta(\omega - \omega_j),
	\]
   where $\rho(\theta, t | \omega)$ governs the probability
   distribution of oscillators with initial value $\omega$.
   It's clear that if $\theta_\omega(t)$ is the solution to
   $(\ref{kuramoto-mf})$, then
   \[
   \rho(\theta, t | \omega) = \delta(\theta - \theta_\omega(t)) \quad \Longrightarrow
   \quad \frac{\partial \rho}{\partial t} = -\frac{\partial}{\partial\theta} \big[\omega + Kr \sin(\psi-\theta)\big]\rho.
	\]
   In the continuum
  \[
