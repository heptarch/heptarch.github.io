...
simulator, with up and down arrows controlling the number of
fireflies, and left and right the size of the coupling. Natural
frequencies and starting phase are randomized:
...
coherence is near zero, as you can check below.
Let's turn on the coupling in our
simulation. For $N = 100$ and $k = 1.1 \times 10^{-4}$, we see
*partial synchronization*. As we increase the coupling, the
synchronization increases until we have total coherence and the
