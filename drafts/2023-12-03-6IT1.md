...
n)^{k-1} = e^{-\lambda}\frac{\lambda^{k-1}}{(k-1)!}
\tag{3} \label{PNT+}
\]
for $\lambda = \log\log n$. This matches a Poisson distribution with parameter $\lambda$.
</div>
...
& = \frac{n}{(k-1)!}\int_{\log 2}^{\log (n/2)} \text{d}s\,
\frac{\log^{k-1}s}{s(log n - s)},
\end{align*} 
where we sum up to $n/2$, <label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
	   ...
	   hence $\pi_(n/x) = 0$.
	   </span> and making the substitution $s = \log (n/x)$.
We can evaluate this by differentiating under the integral
sign. Define
\[
I(\gamma) = \int_{a}^{b} \text{d}s\, \frac{s^{k-1}}{\log n -e^{\gamma s}} = \frac{1}{\log n}\left[\right]^2
\]
</div>
