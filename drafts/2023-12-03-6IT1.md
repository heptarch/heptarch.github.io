...
since phases add perfectly constructively if and only if $\theta_j$ is the
same for all $j$. If they are uncorrelated, they tend to
cancel and give an answer near zero.

<figure>
    <div style="text-align:center; padding: 10px"><img src
    ="/img/posts/firefly1.svg" width="300"/>
	</div>
	</figure>

Let's randomize the natural frequencies $\omega_i$ and starting phase
$\theta_i(0)$ for $N = 100$ fireflies, and visualize the result:

<script src="/assets/fireflies_data/fireflies1.js"></script>

Without any coupling, the fireflies do not flash in sync and the
coherence is near zero.
But with some attraction into the mix!
