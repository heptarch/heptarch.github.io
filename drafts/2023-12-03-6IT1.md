...
\begin{align*}
    ...
    \sim \log \log n,
	 \end{align*}
since $1/\log n \to 0$. The errors take considerably more work, but
...
was proven in 1896 by Hadamard and de la Vallée Poussin independently,
but in 1903, Edmund Landau gave a simpler proof and made a beautiful
generalization. Let $\pi_k(n)$ count
the numbers $1 \leq m\leq n$ with $k$ prime factors, i.e. $\omega(m) =
k$.
Then

$$
\pi_k(n) \sim \frac{1}{(k-1)!}\frac{n}{\log n}(\log\log n)^{k-1}.
\tag{3} \label{PNT+}
$$

The usual PNT is $k=1$.
...
In some versions of the story, Erdős was dozing until Kac made his conjecture!
There are various proofs, but all seem to be unavoidaly technical. So
instead, I'll finish the heuristic<label for="sn-1"
       class="margin-toggle sidenote-number">
</label>
<input type="checkbox"
       id="sn-1"
...
which we can write
\[
\frac{\pi_k(n)}{n} \sim e^{-x}\frac{x^{k-1}}{(k-1)!},
\]
for $x = \log\log x$. 
...
\end{align*}
using $(\ref{CLT})$. This more or less is the non-rigorous argument we
used above, but with Landau's prime Poisson process! $\blacksquare$
</div>
