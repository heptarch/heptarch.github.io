...
oscillators can cause them to synchronize.
Kuramoto (1975) introduced a cute toy model for this coupling which
...
We can randomize these frequencies for $N = 100$ fireflies and
visualize the result. To measure how synchronized they are, we
...
since phases add perfectly coherently if and only if $\theta_j$ is the
same for all $j$. If they are totally random, they tend to
cancel and give an answer near zero.

<script src="/drafts/fireflies_data/fireflies1.js"></script>

Without any coupling, the fireflies do not flash in sync, as we expect.
Let's throw some attraction into the mix!
If firefly $i$ likes to align its phase with firefly $j$, then
$\theta_i$ will tend to slow down if it is ahead of $\theta_j$
($\theta_j - \theta_i < 0$), and speed
up if it lags behind ($\theta_j - \theta_i > 0$).
A simple way to implement this is to add a term to $\dot{\theta}_i$
which is proportional to the difference of phases:
...
This has the same mathematical form as friction. Unfortunately this
leads to exponential damping rather than oscillations.
But observe that when $\theta_j \approx \theta_i$,
$\sin(\theta_j - \theta_i)$ approximates our linear coupling, and
moreover, is always bounded by $1$. So let's try
...
This is the Kuramoto model! Let's turn on the coupling in our
simulation. For $N = 100$ and $kN = 13 \times 10^{-3}$, we see *partial synchronization*:

<script src="/drafts/fireflies_data/fireflies2.js"></script>
