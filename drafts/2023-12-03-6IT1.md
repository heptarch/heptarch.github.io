...
**December 5, 2023.** *In the first issue of 6IT: how to supervise children; unreliably clairvoyant
demons; why the sky is blue (and why it isn't); a Poisson process for
primes; predicting the end of the world from geometry;*
...
5. <a href="#sec-5"><i>Doomsday from the geometry of priors</i></a>
...
<details>
  <summary>
   <i>Proof.</i>$\,$ (Short version: triangulate, 3-color, and place
...
<details>
  <summary>
<i>Proof.</i>$\,$ (Short version: connect a hole to the outside with two walls.)
  </summary>
  <p>
...
<details>
  <summary>
<i>Proof.</i>$\,$ (Short version: prisoners all see the same sequence
eventually, and use this to all guess the same sequence.)
  </summary>
  <p>
...
<details>
  <summary>
<i>Proof.</i>$\,$ (Short version: the demon uses the prisoner's trick
...
<details>
  <summary>
<i>Calculation.</i>$\,$ (Short version: take a product of spectra, differentiate, and set to zero.)
  </summary>
  <p>
...
<details>
  <summary>
<i>Proof.</i>$\,$ (Short version: integrate by parts and use the prime
number theorem.)
  </summary>
  <p>
...
<details>
  <summary>
<i>Proof.</i>$\,$ (Short version: remove a prime from the set, use
...
This is one of the many gems in Kac's
...
<details>
  <summary>
<i>Proof.</i>$\,$ (Short version: apply the CLT to Landau's Poisson distribution.)
  </summary>
  <p>
...
Kac said that the primes play a game of chance; maybe so, but Landau's
beautiful formula tells us that in the long run, their strategies are predictable.
...
## 5. <a href="#tbc">Doomsday from the geometry of priors</a><a id="sec-5" name="sec-5"></a>

When will human civilization end? According to the
[Doomday Argument](https://en.wikipedia.org/wiki/Doomsday_argument),
we can state with $95\%$ confidence it will end in under $10,000$ years.
The idea is simple; suppose that $N_\infty$ humans in total are born. If $N$
have been born to date, how far through the total of $N$ are we?
Let $x = N/N_\infty$ be the fraction that have been so far. There is no
reason to think we are at some special point, so $x \in (0, 1)$ should
be distributed uniformly. Thus, there is a $95\%$ chance that
$x> 0.05$, i.e. that $N_\infty < N/0.05 = 20 N$.
If $N = 60$ billion, then, according to this argument, with $95\%$
confidence we expect at most $\sim 10^{12}$ additional humans will be
born. If the current world population remains stable, that will take
around $30\times 10^{12}/10^{10} = 3000$ years.
