...
distributed. More precisely, let $K_n(\alpha, \beta)$ denote the
number of integers $1 \leq m\leq n$ such that
\[
\log \log n +\alpha\sqrt{\log \log n} <  \omega(m) < \log \log n +\beta\sqrt{\log \log n}.
\]
Dividing by $n$ gives the empirical probability that $\omega(m)$ is
within these bounds. As $n\to \infty$, this probability is normally distributed:
\[
...
<div style="background-color: #16171c ; padding: 20px; border: 0px solid
grey; line-height:1.5">
<i>Proof (heuristic).</i> First, let's consider a Poisson variable
$X \sim \text{Poiss}(x)$. Then, as $x \to \infty$, the CLT implies
that $X \sim \mathcal{N}(x, x)$, and hence
\[
P_{X \sim \text{Poiss}}(x + \alpha\sqrt{x} < X <x + \alpha\sqrt{x} ) =P_{X \sim \mathcal{N}}(x + \alpha\sqrt{x} < X <x + \alpha\sqrt{x})
\]
or, introducing explicit formulas,
\[
