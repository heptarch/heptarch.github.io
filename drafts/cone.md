...
Consider a local perturbation to the magnetic field, $\delta B(\mathbf{r})$, to the background magnetic field. The total number of zero-energy states is determined by the total flux threading the system,

$$
\Phi_\text{tot} = \iint B(\mathbf{r})\, \text{d}^2r
$$
