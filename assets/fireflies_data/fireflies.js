var firefly1 = function(p) {
	...
	move() {
            ...
            if (p.random(0, 1) < 0.4) {
		let angleChange = p.random(-PI / 8, PI / 8);
	...
	display() {
            // Glow effect
            let pulse = p.constrain((4 * sin(this.pulsePhase) - 3), 0, 1);
            ...
            // Draw the glow
            p.fill(255, 255, 100, glowAlpha1); // Soft yellow with alpha for glow
            p.noStroke();
            p.ellipse(this.position.x, this.position.y, glowSize1, glowSize1);
            p.fill(255, 255, 100, glowAlpha2); // Soft yellow with alpha for glow
            p.noStroke();
            p.ellipse(this.position.x, this.position.y, glowSize2, glowSize2);
	    
            // Draw the firefly
            p.fill(255, 255, 0, pulse * this.brightness * 255);
            p.ellipse(this.position.x, this.position.y, this.rad * 2, this.rad * 2);
	}
    }
    ...
    function setup() {
	bgImage = p.loadImage('/assets/fireflies_data/bg1.png');
	...
	for (let i = 0; i < numFireflies; i++) {
		...
		sum[i] += p.sin(fireflies[j].pulsePhase - fireflies[i].pulsePhase);
	    }
	}
	
	// Calculate phase coherence
	for (let i = 0; i < numFireflies; i++) {
	    sines += p.sin(fireflies[i].pulsePhase);
	    cosines += p.cos(fireflies[i].pulsePhase);
	}
	
	coherence = (p.pow(sines, 2) + p.pow(cosines, 2)) / p.pow(numFireflies, 2);
...
var myp5 = new p5(firefly1, "firefly1")
