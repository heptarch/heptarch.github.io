...
for each $t \leq T$, and if $\Delta$ is small enough, we keep it. Thankfully, the update rule is not generally invertible (in particular, for the asymmetric threshold update)
