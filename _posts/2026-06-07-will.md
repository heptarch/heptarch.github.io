...
Brains are complex systems with extraordinarily complex dynamics that are plausibly chaotic in this sense.
If true, this suggests that short-term dynamics on the order of the Lyapunov time $\lambda^{-1}$ is essentially unpredictable (since errors grow exponentially), so simulators cannot have analytic control over brain states for initial conditions specified with finite precision.
