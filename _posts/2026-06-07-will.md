...
for their desired $\sigma$ and a fixed $\sigma_0$. This is a bounded horizon control problem. I don't know how hard this problem is. But I do know that it is easy if $f$ is invertible, since then we simply compute

$$
\Delta = f^{(-t)}[\sigma] - \sigma_0
$$

for each $t \leq T$, and if $\Delta$ is small enough, we keep it.
