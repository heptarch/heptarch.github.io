...
Here, $T$ is some fixed, decisional time scale, plausible $T = \mathcal{O}(1)$ or polynomial at most in $|V|$ so there is a polynomial time algorithm for checking if states evolve into other states: just run the update rules.
