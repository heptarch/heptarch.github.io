...
**June 7, 2026.** *If we live in a simulation, could simulators control our minds? I explain how a form of "computational free will" is protected by complexity theory.*
...
It's plausible to take $T = \mathcal{O}(1)$ or at most polynomial in $\vert V\vert$, so there is a polynomial time algorithm for checking if states evolve into other states: just run the update rules. As mentioned above, the $\textsf{PSPACE}$-completeness only arises when we allow for run times which are *exponential* in $\vert V\vert$, so on the order of $10^{10^{10}}$ time steps, which is much longer than the age of universe when converted to neuronal time scales (milliseconds per step).
