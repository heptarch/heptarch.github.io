...
$$
Y = \lambda f. (\lambda x. f (x\, x))(\lambda x. f (x\, x)), \quad Y f = f (Y f).
$$

The Y-combinator applies a function to the Y of itself; this can easily lead to runaway empty self-reference, while $\text{löb}$ instead gives

$$
\text{löb} f = f (\text{next}(\text{löb} f))
$$

with $\text{next}$ advancing computational time, forcing progress in the recursive stream.
