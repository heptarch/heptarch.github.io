...
8. <a href="#sec-8"><i>Appendix A: Log-likelihood update</i></a><br>
...
## <a href="#toc">Appendix A. Log-likelihood update</a><a id="sec-8" name="sec-8"></a>

The update rule in <a href="#sec-6">§6</a> applies only when the opponent spots first. When you spot first, information lies in silence, i.e., every dog they passed exceeded their threshold, leaving only the one-sided bound
...
with point estimate $\hat b = \alpha / \lambda$.
This is distinct from the mean update rule in <a href="#sec-6">§6</a>. The second sum is the delicate part, since
...
$$
\frac{f}{b} - \sum_j \frac{(1 - \beta_{\min, j})^b\,\log(1 - \beta_{\min, j})}
{1 - (1 - \beta_{\min, j})^b}
\;=\; -\sum_i \log(1 - \beta_i)
$$

numerically, with $f$ the number of opponent-first games. Each first-spotter
