...
<input type="checkbox"
       ...
       class="margin-toggle"/>
	   <span class="sidenote">
This is contentious! Clearly we can't have wingspan $w > W$, so we need to adjust the wingspan. Unlike football, where the goal is much wider than it is long, in hockey the aspect ratio is closer to a square, and areally, a goalie takes up about half the goal. So we project the same proportion to 1D.</span>
- $\tau = 0.2\text{ s}$;
- $v = 3 \text{ m/s}$;
- $V' = 45 \text{ m/s}$
