...
What is genuinely different about Yamaguchi and Kempf's construction is that multiple copies get encrypted and only one can be deciphered; efforts to generalize this to teleportation lead to rather different-looking techniques like [port-based teleportation](https://arxiv.org/abs/1612.09260) which are structurally distinct from the standard protocol, so I won't go into them here. But it's neat that what is perhaps the simplest quantum protocol is also the simplest example of quantum encrypted cloning!
